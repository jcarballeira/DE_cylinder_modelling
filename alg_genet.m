function[bestmem,error,population]=alg_genet(ptCloud,version_de,population,iter_max,max,min,NP,D,F,CR)
%--------------------------------------------------------------------------
%   Function: alg_genet
%--------------------------------------------------------------------------
...
for i=1:NP
       [population(i,1),r_ini]=fitness(ptCloud,population(i,:)); 
end
%--------------------------------------------------------------------------
while (count<=iter_max)%&&(error~=error_max)%&&(error_max>error+10)%3*NP/2) %&&(error>NS/3))%%&&(count_c<30)&&((count_a<100)||(count_b<100)))
    ...
    for i=1:NP
        ...
        [error_trial,r_trial]=fitness(ptCloud,trial);
...
%     error_global=sum(population(:,1)); 
    ...
    if imp_count==200     
        ...
        fprintf(1,'\n It: %f Best %f \n Position: x y z %f %f %f\n Vector: u v w %f %f %f',count,error,population(1,2),population(1,3),population(1,4), population(1,5),population(1,6),population(1,7));
        imp_count=0;
